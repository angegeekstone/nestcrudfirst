...
export class AuthService {
  ...
  async Signin(signinDto: SigninDto) {
    ...
    if(!match) throw new UnauthorizedException('password does not match')
     ...
     const  token = this.jwtService.sign(payload, {expiresIn : '2h', secret: this.configService.get('SECREY_KEY')})
    return {
      token,
      user : {
 ...
 async  resetPassword(resetPasswordDto: ResetPasswordDemandDtp) {
